...
<%!
from dactylo import model, urls
%>
...
<%def name="breadcrumb_content()" filter="trim">
            ...
            <li><a href="${activity.get_admin_url(ctx)}">${activity.get_title(ctx)}</a></li>
            <li class="active">${_(u'Delete')}</li>
</%def>


<%def name="container_content()" filter="trim">
        <h2>${_(u'Deletion of {}').format(activity.get_title(ctx))}</h2>
        <p class="confirm">${_(u"Are you sure that you want to delete this activity?")}</p>
        <form method="post" action="${activity.get_admin_url(ctx, 'delete')}">
            <%view:view_fields/>
...
${_(u'Delete')} - ${activity.get_title(ctx)} - ${parent.title_content()}
</%def>
