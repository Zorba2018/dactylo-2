...
class Activity(objects.Initable, objects.JsonMonoClassMapper, objects.Mapper, objects.ActivityStreamWrapper):
    collection_name = u'activities'
    value = None
...
#                self.title,
    ...
    @classmethod
    def get_admin_class_full_url(cls, ctx, *path, **query):
        return urls.get_full_url(ctx, 'admin', 'activities', *path, **query)

    @classmethod
    def get_admin_class_url(cls, ctx, *path, **query):
        return urls.get_url(ctx, 'admin', 'activities', *path, **query)
    ...
    def get_class_full_url(cls, ctx, *path, **query):
        return urls.get_full_url(ctx, 'activities', *path, **query)

    @classmethod
    def get_class_url(cls, ctx, *path, **query):
        return urls.get_url(ctx, 'activities', *path, **query)
    ...
    def make_id_or_slug_or_words_to_instance(cls):
        def id_or_slug_or_words_to_instance(value, state = None):
            ...
            if state is None:
                state = conv.default_state
            match = conv.object_id_re.match(value)
            if match is None:
#                self = cls.find_one(dict(slug = value), as_class = collections.OrderedDict)
                self = None
            else:
                self = cls.find_one(bson.objectid.ObjectId(value), as_class = collections.OrderedDict)
            if self is None:
                words = sorted(set(value.split(u'-')))
                instances = list(cls.find(
                    dict(
                ...
                if not instances:
                    return value, state._(u"No activity with ID, slug or words: {0}").format(value)
                if len(instances) > 1:
                    return value, state._(u"Too much activities with words: {0}").format(u' '.join(words))
                self = instances[0]
            return self, None
        return id_or_slug_or_words_to_instance
...
#    Activity.ensure_index('slug', unique = True)
    Activity.ensure_index('updated')
    Activity.ensure_index('words')
